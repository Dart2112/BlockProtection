	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("bptool")) {
			...
			if (!p.hasPermission("bp.admin")) { //No permissions
				...
				if (BPConfigHandler.advLog()) pl.log.warning(bpp + " attempted to use command: 'bptool'.");
			} 
			else {
				...
				if (BPConfigHandler.advLog()) pl.log.info(bpp + " has been given a utility tool."); //Log to console
				return true;
			}
		}
		return true;
