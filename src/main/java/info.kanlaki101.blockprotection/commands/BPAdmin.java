	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("bpadmin")) {
			...
			if (!p.hasPermission("bp.admin")) {
				...
				if (BPConfigHandler.advLog()) pl.log.warning(player + " attempted to use command: 'bpadmin'."); //Log attempted use of command
				return true;
			}
			if (pl.UsersBypass.contains(player)) { //Check for the player in the bypass protection array
				...
				if (BPConfigHandler.advLog()) pl.log.info(player + " is no longer bypassing BlockProtection"); //Log to console
			}
			else {
				...
				if (BPConfigHandler.advLog()) pl.log.info(player + " is now bypassing BlockProtection"); //Log to console
			}
		}
		return true;
	}
